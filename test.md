...
for i in range(len(chunk_dict)):
    freq=chunk_dict[i]['AccountName'].value_counts(dropna=False)
    print(freq.head(20))
...
fig.update_layout(
    ...
    title="Long-tail Distribution of AccountName",
    xaxis_title="AccountName Rank (sorted by frequency)",
...
print('df.dtypes')
print(df.dtypes)
print("df[['AccountName', 'count_pct']].head(10)")
print(df[['AccountName', 'count_pct']].head(10))
dfs = []
...
plt.ylabel("Number of AccountName")
